...
> **Behavior change (all `migrate add` users).** Canonical name matching is *always on* in the core differ, not just in `adopt`. An add+drop pair whose names are equal under canonicalization (case/separator-only, e.g. `user_name`↔`UserName`) is now always emitted as a `RENAME COLUMN`/`RENAME TABLE` and is **not** offered to the interactive/`acceptRename` confirmation — so a convention-only rename can never degrade to drop+add. This strictly prevents data loss, but if you genuinely intend to drop a column and add a canonically-equal one (discarding the old data), split it across two migrations or use `--allow-data-loss` on a DB-sourced diff.

**Building blocks.** `migrate adopt` is `migrate baseline --from-database` (record the live DB state as an applied baseline) followed by a normalized `--from-database` diff against your project schemas. Those pieces are available standalone: `migrate baseline <name>` (record-as-applied) and `migrate add/diff --from-database "…"` (diff against a live database rather than the last snapshot). **Prefer `adopt`, or always pass `--from-database` for the follow-up diff.** A `baseline --from-database` persists a snapshot carrying live-database metadata (identity/length/defaults/FKs/indexes) that the project-schema reader does not populate; a subsequent *plain* `migrate add` (no `--from-database`) diffs those against your schemas and reports spurious per-column `AlterColumn`s. `adopt` and `--from-database` avoid this by normalizing both sides to the reliably-shared column subset.
