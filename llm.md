...
- **Adopting an existing database** — `quarry migrate adopt <name> --connection "…" -d <dialect>` introspects the live DB, records its current state as an already-applied baseline (`InitialCreate`, no DDL run), and generates one *pending* alignment migration that transforms the DB to match your schemas. `migrate baseline <name>` does just the record-as-applied half; `migrate add`/`diff` accept `--from-database "…"` to diff against a live DB instead of the last snapshot. Renames are detected deterministically (see below) or declared with `--rename-map "table.col=New,…"` / `--rename-map @renames.csv`; a drop against populated data is refused unless `--allow-data-loss` is passed. See [docs/articles/migrations.md](docs/articles/migrations.md).
- **Convention-aware rename detection** — the differ matches added/dropped columns and tables whose names are equal under canonical normalization (case- and separator-insensitive: `user_name` ≡ `userName` ≡ `UserName`) as deterministic renames, before any heuristic scoring. This is always on, so a snake_case→PascalCase change emits `RENAME COLUMN` (data-preserving) rather than drop+add.
