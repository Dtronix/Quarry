...
- **Do not inline a worker body into an async lambda inside a loop.** `ConcurrencyTests` keeps each worker in a named `private static async Task<T> Run…WorkerAsync(...)` method. Chains inside lambdas are supported now, but for *this* shape — an async lambda inside a loop whose clause captures a local — the predicted display-class name is not stable across compiler versions: the same source yielded `<>c__DisplayClass5_3` on SDK 10.0.110 and `<>c__DisplayClass5_1` on SDK 10.0.302, so CI failed with `TypeLoadException` while the identical run passed locally. A named method turns the captures into ordinary method locals, which predict stably. Tracked as issue #344.
