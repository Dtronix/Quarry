...
**When you add an emitter path, add a shape for it.** And add a `RuntimeHelperExpectations` entry naming the runtime member that shape is supposed to emit — `Shape_StillReachesItsRuntimeHelper` pins it. Without that entry a shape that quietly stops exercising its emitter (a conditional that stops being analyzable, a collection no longer routed through the SQL cache) keeps passing while guarding nothing, and a green run cannot tell the difference. That check caught exactly this on its first run.
