...
- 2026-07-14 Step 5 sweep discovery: `TypeMappingRegistry.TryConvert`/`TryConfigureParameter` (src/Quarry/Mapping/TypeMappingRegistry.cs) have NO production callers — the doc-claimed consumer `NormalizeParameterValue` does not exist anywhere in src. The whole runtime-fallback parameter-conversion path (incl. related comments in TypeMapping.cs:26 and IDialectAwareTypeMapping.cs:12) is dead machinery from the removed runtime-fallback model, kept alive only by TypeMappingRegistryTests. Removing runtime-library code is out of #311's text-sweep scope — candidate for a separate issue at REVIEW (class C). Left untouched in Step 5.
- 2026-07-14 Step 5 note: QRY001/QRY019 remain Warning severity while their consequence is now a runtime InvalidOperationException — arguably should be Errors. Out of scope for the text sweep (severity change = behavior change); worth surfacing in REVIEW.
- 2026-07-14 Step 5: docs/articles/releases/release-notes-v0.4.0.md quotes the old QRY019 text — historical record of what v0.4.0 shipped, intentionally NOT updated.
...
| 2026-07-14 | IMPLEMENT, REVIEW, REMEDIATE | Steps 4–6 committed. Review: 15 findings (11A/3B/1D after user reclassified F3 C→A). All A+B remediated incl. orphan-diagnostic group, QRY001/QRY019→Error, dead TypeMappingRegistry removal. Rebased onto ad0af63, 3411+146+201 tests green. PR #326 created; CI watch started. |
