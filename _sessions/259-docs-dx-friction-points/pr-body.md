...
- Phase 2: Nested row type support via new `IsNestedType` / `FullyQualifiedResultTypeName` fields on `RawSqlTypeInfo`. Emitters branch on `IsNestedType` to pick the FQN; struct-reader identifier is sanitized.
...
### Internal
- `RawSqlTypeInfo` ctor gains two optional parameters (`isNestedType = false`, `fullyQualifiedResultTypeName = null`). All existing callers compile unchanged and opt into the default.
