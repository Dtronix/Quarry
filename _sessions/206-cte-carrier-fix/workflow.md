...
**2026-04-06: Conditional `With<>`/`FromCte<>` handling — emit QRY083 diagnostic**
The chain analyzer treats `CteDefinition` like any other clause for conditional handling (`MapInterceptorKindToClauseRole` includes it), but the `CteDefinition` build loop adds CTEs to the plan unconditionally regardless of `NestingContext`. This means an `if (cond) db.With<X>(...)` already produces broken SQL (the `WITH` clause renders even when the call doesn't execute) — a pre-existing latent bug. The first-vs-subsequent emission strategy in this fix would introduce a new failure mode in the same latent area: if the source-order first `With<>` is conditionally skipped at runtime but a later `With<>` runs, our emitted `Unsafe.As<{carrier}>(@this)` would reinterpret the real context as a carrier (undefined behavior). Add a new diagnostic `QRY083` rejecting `CteDefinition` and `FromCte` sites with non-null `NestingContext` to make the latent bug visible AND prevent the new failure mode. No existing tests exercise conditional CTE, so this can ship cleanly.
