...
- [x] **3. `--rename-map` parsing + forced-rename pre-transform.**
...
- [x] **4. Migration-history writer + checksum access.** (Note: Quarry->Quarry.Tool AND Quarry.Tests IVT already exist in Quarry.csproj, so ComputeChecksum was already reachable — no IVT change needed.)
...
- [x] **5. `migrate baseline <name>` command.** (Implemented as MigrateCommands.MigrateBaseline method, not a separate class — see workflow.md deviations.)
...
- [x] **6. `--from-database` on `migrate add` and `migrate diff`.** (Shared ResolveFromSnapshotAsync helper; default path provably unchanged. Data-path tested via DatabaseSchemaReader+SchemaDiffer composition.)
