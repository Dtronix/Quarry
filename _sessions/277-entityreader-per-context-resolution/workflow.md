...
| 1 | REMEDIATE | REVIEW | User-triggered back-step from REMEDIATE → REVIEW after questioning the EntityReaderIntegrationTests.cs deletion and the IQueryBuilder<TIn, TOut> shape. Both questions answered satisfactorily on context, but user requested a full re-analysis of the diff. Preserved pass-1 review at `review-pass1.md`; PR #278 left open; decisions table kept; phases-complete unchanged at 6. Delegating a full second-pass analysis. |
| 1 | REVIEW | REMEDIATE | Pass-2 analysis surfaced 1 Medium + 3 Low findings. User classified as 4A / 0B / 0C / 1D ("C->A Implement all A&C Now"). Auto-transition to REMEDIATE for implementation. |
| 1 | REMEDIATE | REMEDIATE | Item A1 (nested-type `+` separator): updated `ResolvePerContextReaderFqn` to split on the last `.` or `+`. Item A2 (chain-shape assertion): added regex assertions to existing generator test for `<App.Pg.Product, App.Pg.Product>` and `<App.My.Product, App.My.Product>`. Item A4 (negative test for missing reader): added `Generator_PerContextEntityReader_MissingPerContextReader_ProducesCompileError` that verifies CS0234/CS0246 surfaces against the missing per-context reader class. Item A5 (set-op/CTE coverage): added `Union_IdentityProjection_UsesCustomReader` and `Cte_FromCte_IdentityProjection_UsesCustomReader` to `CrossDialectEntityReaderTests.cs`. Joined-identity case skipped — Product has no FK in existing fixtures. Tests: 3345/3345 passing (was 3342, +3 from new tests). |
