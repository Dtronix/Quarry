...
| 4 | REVIEW | REVIEW | Resumed from suspend. PR #210 still OPEN/MERGEABLE, CI green on `19c2c6a` (the WIP commit had been pushed). User chose "Fix all" for the three review-session3.md findings. Applied: orphaned `<summary>` move in RawCallSite.cs, row-value assertion in `CrossEntity_Union_WithParameters`, `if (rebound != null) break;` in CallSiteBinder rebind loop. 2967 tests green. Amended WIP `19c2c6a` → `9852c36` and force-pushed (workflow's documented WIP-amend pattern). New CI #24055957510 green in 1m35s. |
| 4 | REVIEW | FINALIZE | User chose (B) Rebase from origin/master first. Rebased onto `6df97b0` (master had advanced with #209 DML translation and #208 CTE/derived tables). Resolved conflicts: RawCallSite.cs (CTE fields + OperandEntityTypeName both kept), UsageSiteDiscovery.cs (constructor call append), DiagnosticDescriptors.cs (kept QRY080/QRY081, removed QRY073), all 4 manifest files (took theirs, then regenerated via build). All session 4 fixes survived rebase. 3012 tests green (97 migration + 103 analyzer + 2812 main — +45 from master's added migration/CTE tests). |
