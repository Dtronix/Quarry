...
**Track B (Phases 5–9). Phase 10 partially scoped, suspended after architectural investigation.**
...
- Phase 10 (suspended mid-investigation): Discovered a pre-existing generator bug in the `[EntityReader]` attribute resolution. Per-context entity types (`Pg.Product`, `My.Product`, `Ss.Product`) are separate generated classes that lack the global `Product`'s DisplayLabel partial extension. The interceptor codegen emits the reader's `T` as the global Product even when the call site expects the per-context type, surviving compilation only via `Unsafe.As<>` casts. User chose **Layer 1 + Layer 2** fix scope (test-side per-context partials + readers AND generator-side per-context name lookup for `[EntityReader]`). Detailed plan recorded in `workflow.md` Suspend State. Reverted to clean Phase 9 baseline; no Phase 10 code committed yet.
