...
| 3 | IMPLEMENT P11 | IMPLEMENT P11 | Phase 11 complete — converted RawSqlIntegrationTests.cs (17 SQLite-only tests) to CrossDialectRawSqlTests.cs (19 tests; one for-each-dialect aggregation merged 2 original tests). Per-dialect identifier quoting (`"x"` Lite/Pg, `` `x` `` My, `[x]` Ss); `@p0` placeholders on every dialect (RawSql runtime always assigns `param.ParameterName = "@pN"`, which Npgsql rewrites to positional internally — confirmed via QuarryContext.cs line 185 docstring). Deleted Integration file. Tests: 3351/3351 (Quarry.Tests 3023 unchanged — net wash from 4× execution per converted test). |
| 3 | IMPLEMENT P12 | IMPLEMENT P12 | Phase 12 complete — converted LoggingIntegrationTests.cs (29 tests) to CrossDialectLoggingTests.cs. Each test runs sequentially on Lite/Pg/My/Ss with `_logger.Clear()` between dialects (LogsmithOutput.Logger is a process-wide singleton, fixture is `[NonParallelizable]`). Connection-lifecycle tests build fresh per-dialect connections from each test container's `GetConnectionStringAsync`. Sensitive-redaction tests (3) intentionally remain SQLite-only with explicit per-test SqliteConnection setup — Pg/My/Ss baselines do not seed the widgets table, and the redaction code path runs in QuarryContext before SQL is built so single-dialect verification is sufficient. Fixed one parameter type mismatch (Pg TIMESTAMP rejects string-typed CreatedAt; switched to DateTime). Deleted Integration file. Tests: 3351/3351 (Quarry.Tests 3023 unchanged). **Track B complete.** |
