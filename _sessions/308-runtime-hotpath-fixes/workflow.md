...
- **Step 4 done (2026-07-13):** 3 edits — (1) `GeneratePropertyAssignment` emits `{GetMappingFieldName(m)}.FromDb(...)`; (2) `EmitRowReaderStruct` emits struct-local `static readonly {m} {field} = new();`; (3) `CollectMappingInstances` walks `RawSqlTypeInfo.Properties` for the lambda readers' file-scope fields. RawSql mapping attaches only when the generic arg is a REGISTERED ENTITY (`entityRegistry.Resolve`), matched by property name to schema columns (`PatchWithColumnMetadata`) — NOT for plain DTOs. Field-name format confirmed `_mapper_TestApp_MoneyMapping` (no `global::`). **Updated pre-existing test** `RawSqlInterceptorTests.RawSqlAsync_EntityType_WithCustomTypeMapping_GeneratesFromDb` which pinned the old `new MoneyMapping().FromDb(...)` form → now asserts cached `_mapper_MoneyMapping.FromDb(...)` + struct field + no `new().FromDb`. Full suite 3287 green.
