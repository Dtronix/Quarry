...
- 2026-07-02 (REMEDIATE pass 2): QRY048 registered in s_deferredDescriptors (was silently dropped); TryMergeTextOrder replaced by TryMergeTextOrders — topological merge with GlobalIndex tiebreak per original plan (anchor-insertion false-contradiction fixed); zero-marker variants now validated (whole-surface marker miss ⇒ QRY048, not silent identity). Tests per user directive: QRY048 end-to-end surfacing test (marker-shaped string constant in UPDATE SET — top-level WHERE string constants parameterize, SET constants inline), 2-conditional×wrap generation test, and 3 new MySQL execution tests (2-conditional×wrap all 4 masks; parameterized-CTE inner param; parameterized Union operands). Discovery while testing: pre-existing-suspect generator defect QRY037 — CTE chain with captured inner param + captured outer param (Where after FromCte) declares carrier P1 but never assigns it; being verified against master for a separate issue; the new CTE test keeps its outer filter literal to avoid it.
- 2026-07-02 (post-CI, user follow-up): QRY037 scoped empirically — outer-param-only and two-inner-param CTE shapes build cleanly (QRY030); only the inner+outer combination fails. Added 2 more MySQL execution tests (ParameterizedCteTwoInnerParams, ParameterizedCteOuterParam) so every buildable parameterized-CTE shape is execution-pinned; the inner+outer ordering pin is blocked by QRY037 (build error ⇒ no runtime exposure) and is required deliverable of the QRY037 fix (recorded in _issue-body.md).
