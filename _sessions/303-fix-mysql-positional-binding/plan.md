...
- **Validation failure handling** (post-review remediation of findings #1/#6): extraction/validation failure now emits warning **QRY048** at the chain's terminal (with a reason) in addition to the identity fallback; the bare `Debug.Assert` was removed. *Pass-2 correction (2026-07-02):* the descriptor was initially never registered in `s_deferredDescriptors`, so every QRY048 was silently dropped — registration added, guarded end-to-end by `MarkerShapedStringLiteral_MySQL_SurfacesQRY048_AsWarning`.
- **Cross-variant merge algorithm** (pass-2 remediation of findings #2/#4, 2026-07-02): the plan's "insertion-merge ... GlobalIndex tiebreak" landed as an insert-after-anchor guess, which falsely reported "contradictory placeholder order" for ≥2 independently conditional parameterized clauses (mask enumeration feeds `[0]`, `[1]` before `[0,1]`) and silently fell back to identity — re-shipping the #303 misbind for wrap chains of that shape. Replaced by `TryMergeTextOrders`: topological sort over pairwise constraints with smallest-slot-first (GlobalIndex) tiebreak, exactly as planned. Unit-tested (`MySqlBindOrderMergeTests`, incl. the singleton-then-combined family), generation-tested (`TwoIndependentConditionalFilters_WrapChain_...`), and execution-tested on MySQL (all four mask variants).
- **Zero-marker validation** (pass-2 remediation of finding #5, 2026-07-02): marker-free variants no longer bypass validation — a variant with active parameters but zero extracted markers (an entire render surface missing marker emission) now fails to QRY048 instead of degrading to silent identity.
...
- **Pagination placeholder slots** (post-review remediation of finding #7, scope widened at user direction): `AppendPagination` now uses `PaginationPlan.LimitParamIndex`/`OffsetParamIndex` (true global slots) on **all dialects**, not just for MySQL markers. The clause-level running index lags the true slot when projection params exist, which produced `$N`/`@pN` collisions with projection placeholders — on PostgreSQL, `LIMIT` silently received a window-function argument's value; on SQLite/SqlServer the bound name missed the placeholder. Identical output for all chains without projection params. Covered by a 4-dialect SQL-output + execution test (`WindowFunction_ParamArgs_WithParameterizedLimit_NumbersPaginationByGlobalSlot`).
