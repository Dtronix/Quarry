...
- [x] **13. F4c — Row-order sweep: remaining ~17 files + First/FetchFirst ORDER BY remediation** *(split into two commits. 13a: 31 sites across the remaining 21 files — 16 of them needed nothing, see Working Notes. 13b: six query-side ORDER BY fixes (4 pagination + FetchFirst-on-multi-row + Where pagination), and the nine unfixable JoinTests filed as **#332** per user decision rather than changed, with a `<remarks>` pointer added in the file. Manifest goldens regenerated and committed.)*
