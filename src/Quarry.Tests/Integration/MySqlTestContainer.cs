...
internal static class MySqlTestContainer
{
    ...
    public static async Task EnsureBaselineAsync()
    {
        ...
        try
        {
            ...
            await TestContainerHelpers.ExecAsync(conn,$"SELECT GET_LOCK('{BaselineLockName}', 60);");
            try
            {
                ...
                var alreadyReady = await TestContainerHelpers.TableExistsAsync(conn, BaselineDatabaseName, "users");
                if (!alreadyReady)
                {
                    await TestContainerHelpers.ExecAsync(conn,$"USE `{BaselineDatabaseName}`;");
            ...
            {
                await TestContainerHelpers.ExecAsync(conn,$"SELECT RELEASE_LOCK('{BaselineLockName}');");
            }
    ...
    public static async Task<string> CreateOwnedDatabaseAsync(MySqlConnection connection)
    {
        ...
        await TestContainerHelpers.ExecAsync(connection, $"CREATE DATABASE `{database}`;");
        await TestContainerHelpers.ExecAsync(connection, $"USE `{database}`;");
    ...
    private static async Task CreateSchemaObjectsAsync(MySqlConnection conn)
    {
        ...
        await TestContainerHelpers.ExecAsync(conn,@"CREATE TABLE `orders` (
        ...
        await TestContainerHelpers.ExecAsync(conn,@"CREATE TABLE `order_items` (
        ...
        await TestContainerHelpers.ExecAsync(conn,@"CREATE TABLE `accounts` (
        ...
        await TestContainerHelpers.ExecAsync(conn,@"CREATE TABLE `events` (
        ...
        await TestContainerHelpers.ExecAsync(conn,@"CREATE TABLE `addresses` (
        ...
        await TestContainerHelpers.ExecAsync(conn,@"CREATE TABLE `user_addresses` (
            `UserAddressId` INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
            `UserId` INT NOT NULL,
            `AddressId` INT NOT NULL
        );");

        await TestContainerHelpers.ExecAsync(conn,@"CREATE TABLE `warehouses` (
            `WarehouseId` INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
            `WarehouseName` TEXT NOT NULL,
            `Region` TEXT NOT NULL
        );");

        await TestContainerHelpers.ExecAsync(conn,@"CREATE TABLE `shipments` (
        ...
        await TestContainerHelpers.ExecAsync(conn,@"CREATE TABLE `products` (
    ...
    {
        await TestContainerHelpers.ExecAsync(conn,@"INSERT INTO `users` (`UserId`, `UserName`, `Email`, `IsActive`, `CreatedAt`, `LastLogin`) VALUES
        ...
        await TestContainerHelpers.ExecAsync(conn,@"INSERT INTO `orders` (`OrderId`, `UserId`, `Total`, `Status`, `Priority`, `OrderDate`, `Notes`) VALUES
        ...
        await TestContainerHelpers.ExecAsync(conn,@"INSERT INTO `order_items` (`OrderItemId`, `OrderId`, `ProductName`, `Quantity`, `UnitPrice`, `LineTotal`) VALUES
            (1, 1, 'Widget', 2, 125.00, 250.00),
            (2, 2, 'Gadget', 1, 75.50,  75.50),
            (3, 3, 'Widget', 3, 50.00,  150.00);");

        await TestContainerHelpers.ExecAsync(conn,@"INSERT INTO `accounts` (`AccountId`, `UserId`, `AccountName`, `Balance`, `credit_limit`, `IsActive`) VALUES
            (1, 1, 'Savings',  1000.50, 5000.00, 1),
            (2, 1, 'Checking', 250.75,  1000.00, 1),
            (3, 2, 'Savings',  500.00,  2000.00, 0);");

        await TestContainerHelpers.ExecAsync(conn,@"INSERT INTO `events` (`EventId`, `EventName`, `ScheduledAt`, `CancelledAt`) VALUES
            (1, 'Launch', '2024-06-15 10:30:00', NULL),
            (2, 'Review', '2024-07-01 14:00:00', '2024-06-28 09:00:00');");

        await TestContainerHelpers.ExecAsync(conn,@"INSERT INTO `addresses` (`AddressId`, `City`, `Street`, `ZipCode`) VALUES
            (1, 'Portland', '123 Main St', '97201'),
            (2, 'Seattle', '456 Oak Ave', '98101');");

        await TestContainerHelpers.ExecAsync(conn,@"INSERT INTO `user_addresses` (`UserAddressId`, `UserId`, `AddressId`) VALUES
            (1, 1, 1),
            (2, 1, 2),
            (3, 2, 1);");

        await TestContainerHelpers.ExecAsync(conn,@"INSERT INTO `products` (`ProductId`, `ProductName`, `Price`, `Description`) VALUES
            (1, 'Widget',  29.99, 'A fine widget'),
            (2, 'Gadget',  49.50, NULL),
            (3, 'Doohickey', 9.95, 'Budget option');");

        await TestContainerHelpers.ExecAsync(conn,@"INSERT INTO `warehouses` (`WarehouseId`, `WarehouseName`, `Region`) VALUES
            (1, 'West Coast Hub', 'US'),
            (2, 'EU Central', 'EU');");
