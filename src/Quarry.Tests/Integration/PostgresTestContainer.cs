...
internal static class PostgresTestContainer
{
    ...
    /// Column types match what <c>Quarry.Migration.SqlTypeMapper</c> emits for
    /// each CLR type on PG, because Npgsql is strict about <c>DataTypeName</c>
    /// when materialising values into typed reader calls.
    /// </summary>
    /// <remarks>
    /// <list type="bullet">
    ...
    ///   <item><description><c>Col&lt;decimal&gt;</c> / <c>Col&lt;Money&gt;</c>→decimal columns use <c>NUMERIC(18, 2)</c>. PG's Npgsql refuses to read <c>double precision</c> as <c>System.Decimal</c>, so SQLite's <c>REAL</c> port to <c>DOUBLE PRECISION</c> won't work.</description></item>
    ///   <item><description><c>Col&lt;bool&gt;</c> columns use <c>BOOLEAN</c>. PG has no implicit integer↔boolean conversion, so <c>INTEGER</c> would break both <c>WHERE col = TRUE</c> filters and <c>reader.GetBoolean(i)</c> materialisation.</description></item>
    ///   <item><description><c>Col&lt;DateTime&gt;</c> uses <c>TIMESTAMP</c>; <c>Col&lt;DateTimeOffset&gt;</c> uses <c>TIMESTAMPTZ</c>. PG <c>TEXT</c> won't materialise into typed <c>DateTime</c>/<c>DateTimeOffset</c> via Npgsql.</description></item>
    ///   <item><description>All identifiers stay quoted to match the SQLite source's case-sensitive naming.</description></item>
    ///   <item><description>FOREIGN KEY constraints from the SQLite source are intentionally omitted. SQLite does not enforce FKs unless <c>PRAGMA foreign_keys = ON</c> is explicitly set, which the test harness does not. Replicating the FKs in PG would diverge from Lite's effective behaviour and break tests that delete/mutate parent rows without first cleaning up children (e.g., delete-by-where-clause tests).</description></item>
    /// </list>
    /// </remarks>
    private static async Task CreateSchemaObjectsAsync(NpgsqlConnection conn, string schema)
    {
        ...
        await ExecAsync(conn, $@"CREATE TABLE {q}.""tags"" (
    ...
    private static async Task SeedDataAsync(NpgsqlConnection conn, string schema)
    {
        ...
        await ExecAsync(conn, $@"INSERT INTO {q}.""users"" (""UserId"", ""UserName"", ""Email"", ""IsActive"", ""CreatedAt"", ""LastLogin"") VALUES
            (1, 'Alice',   'alice@test.com',   TRUE,  '2024-01-15 00:00:00', '2024-06-01 00:00:00'),
            (2, 'Bob',     NULL,               TRUE,  '2024-02-20 00:00:00', NULL),
            (3, 'Charlie', 'charlie@test.com', FALSE, '2024-03-10 00:00:00', '2024-05-15 00:00:00');");
        ...
        await ExecAsync(conn, $@"INSERT INTO {q}.""accounts"" (""AccountId"", ""UserId"", ""AccountName"", ""Balance"", ""credit_limit"", ""IsActive"") VALUES
            (1, 1, 'Savings',  1000.50, 5000.00, TRUE),
            (2, 1, 'Checking', 250.75,  1000.00, TRUE),
            (3, 2, 'Savings',  500.00,  2000.00, FALSE);");
        ...
        await ExecAsync(conn, $@"INSERT INTO {q}.""tags"" (""TagId"", ""OrderItemId"", ""TagName"", ""TagValue"") VALUES
            (1, 1, 'urgent',  'P1'),
            (2, 1, 'fragile', 'yes'),
            (3, 2, 'urgent',  'P1'),
            (4, 3, 'urgent',  'P1'),
        ...
        foreach (var (table, pk) in new[]
        {
            ...
            ("shipments", "ShipmentId"), ("products", "ProductId"), ("tags", "TagId"),
        })
        {
