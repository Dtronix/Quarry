...
";
    ...
    // ── Inline Scalar Terminal Tests ──

    [Test]
    public void CarrierGeneration_InlineScalar_ZeroParams()
    {
        var source = SharedSchema + @"
[QuarryContext(Dialect = SqlDialect.SQLite)]
public partial class TestDbContext : QuarryContext
...
";
        ...
        // Inline scalar: must NOT delegate to QueryExecutor for scalar path
        Assert.That(code, Does.Not.Contain("ExecuteCarrierScalarWithCommandAsync"));
        // Must contain inline ExecuteScalarAsync call
        Assert.That(code, Does.Contain("await __cmd.ExecuteScalarAsync(cancellationToken).ConfigureAwait(false)"));
        // Must contain ScalarConverter.Convert
        Assert.That(code, Does.Contain("ScalarConverter.Convert<TScalar>(__result)"));
        // Must contain try/catch error handling
        Assert.That(code, Does.Contain("throw new QuarryQueryException($\"Error executing scalar query:"));
        // Must contain async modifier on the method
        Assert.That(code, Does.Contain("public static async Task<TScalar>"));
        // Must contain EnsureConnectionOpenAsync inline
        Assert.That(code, Does.Contain("await __ctx.EnsureConnectionOpenAsync(cancellationToken).ConfigureAwait(false)"));
        // Must contain instrumentation gate
        Assert.That(code, Does.Contain("Stopwatch.GetTimestamp()"));
    }

    [Test]
    public void CarrierGeneration_InlineScalar_WithParameters()
    {
        var source = SharedSchema + @"
[QuarryContext(Dialect = SqlDialect.SQLite)]
public partial class TestDbContext : QuarryContext
...
public static class Queries
{
    public static async Task Test(TestDbContext db, bool active)
    {
        var count = await db.Users()
...
";
        ...
        // Inline scalar with parameters: must NOT delegate
        Assert.That(code, Does.Not.Contain("ExecuteCarrierScalarWithCommandAsync"));
        // Must contain inline ExecuteScalarAsync
        Assert.That(code, Does.Contain("await __cmd.ExecuteScalarAsync(cancellationToken).ConfigureAwait(false)"));
        // Must contain parameter binding
        Assert.That(code, Does.Contain("__cmd.Parameters.Add("));
    }

    [Test]
    public void CarrierGeneration_InlineScalar_ConditionalWhere()
    {
        var source = SharedSchema + @"
...
public class Svc
{
    ...
    public async Task<int> Run(bool active, bool applyFilter)
    {
        var q = _db.Users().Select(u => Sql.Count());
        if (applyFilter)
            q = q.Where(u => u.IsActive == active);
        return await q.ExecuteScalarAsync<int>();
    }
}
";
        ...
        // Inline scalar with conditional: must NOT delegate
        Assert.That(code, Does.Not.Contain("ExecuteCarrierScalarWithCommandAsync"));
        // Must contain inline ExecuteScalarAsync
        Assert.That(code, Does.Contain("await __cmd.ExecuteScalarAsync(cancellationToken).ConfigureAwait(false)"));
        // Must contain mask-based dispatch for conditional chains
        Assert.That(code, Does.Contain("__c.Mask"));
    }

    [Test]
    public void CarrierGeneration_InlineScalar_NullHandling()
    {
        var source = SharedSchema + @"
[QuarryContext(Dialect = SqlDialect.SQLite)]
public partial class TestDbContext : QuarryContext
...
";
        ...
        // Must contain null/DBNull handling
        Assert.That(code, Does.Contain("__result is null or DBNull"));
        Assert.That(code, Does.Contain("default(TScalar) is null"));
        Assert.That(code, Does.Contain("throw new InvalidOperationException"));
    }
}
