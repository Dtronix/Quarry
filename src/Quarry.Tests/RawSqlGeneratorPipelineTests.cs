...
";
        ...
        // Interceptor should be generated for the entity type with a proper reader delegate
        Assert.That(code, Does.Contain("RawSqlAsyncWithReader"),
        ...
        // Entity enrichment should populate properties from schema metadata
...
";
    ...
    public void RawSqlAsync_EntityT_SimpleColumns_GeneratesPropertySwitch()
    {
        ...
        // from schema metadata and generate a switch-based reader, not a no-op lambda.
        var source = @"
using Quarry;
using System.Threading.Tasks;
...
";
        ...
        // Enrichment should produce a reader with property switch cases
        Assert.That(code, Does.Contain("switch (r.GetName(i))"),
            "Should generate switch-based reader for enriched entity type");
        Assert.That(code, Does.Contain("case \"UserId\""),
