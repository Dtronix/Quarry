...
public class InterceptorIntegrationTests
{
    ...
    #region Generated Metadata Tests

    [Test]
    public void UserMetadata_TableName_IsCorrect()
    {
        Assert.That(UserMetadata.TableName, Is.EqualTo("users"));
    }

    [Test]
    public void UserMetadata_QualifiedTableName_IsQuoted()
    {
        Assert.That(UserMetadata.QualifiedTableName, Is.EqualTo("\"users\""));
    }

    [Test]
    public void UserMetadata_ColumnCount_MatchesProperties()
    {
        Assert.That(UserMetadata.ColumnCount, Is.EqualTo(6));
    }

    [Test]
    public void UserMetadata_ColumnNames_ContainsAllColumns()
    {
        var columnNames = UserMetadata.ColumnNames;

        Assert.That(columnNames, Does.Contain("UserId"));
        ...
        Assert.That(columnNames, Does.Contain("LastLogin"));
    }

    [Test]
    public void UserMetadata_PrimaryKey_IsUserId()
    {
    ...
    [Test]
    public void UserMetadata_NullableColumns_MarkedCorrectly()
    {
        Assert.That(UserMetadata.Columns.Email.IsNullable, Is.True);
        Assert.That(UserMetadata.Columns.LastLogin.IsNullable, Is.True);
        Assert.That(UserMetadata.Columns.UserName.IsNullable, Is.False);
        Assert.That(UserMetadata.Columns.IsActive.IsNullable, Is.False);
    }

    [Test]
    public void UserMetadata_MaxLength_SetCorrectly()
    {
        Assert.That(UserMetadata.Columns.UserName.MaxLength, Is.EqualTo(100));
    }

    [Test]
    public void UserMetadata_ColumnToProperty_MappingExists()
    {
        Assert.That(UserMetadata.ColumnToProperty["UserId"], Is.EqualTo("UserId"));
        Assert.That(UserMetadata.ColumnToProperty["UserName"], Is.EqualTo("UserName"));
    }

    [Test]
    public void UserMetadata_PropertyToColumn_MappingExists()
    {
        Assert.That(UserMetadata.PropertyToColumn["UserId"], Is.EqualTo("UserId"));
        Assert.That(UserMetadata.PropertyToColumn["UserName"], Is.EqualTo("UserName"));
    }

    [Test]
    public void OrderMetadata_TableName_IsCorrect()
    {
        Assert.That(OrderMetadata.TableName, Is.EqualTo("orders"));
    }

    [Test]
    public void OrderMetadata_ForeignKey_MarkedCorrectly()
    {
    ...
    [Test]
    public void OrderItemMetadata_TableName_IsCorrect()
    {
        Assert.That(OrderItemMetadata.TableName, Is.EqualTo("order_items"));
    }

    [Test]
    public void OrderItemMetadata_ForeignKey_MarkedCorrectly()
    {
        Assert.That(OrderItemMetadata.Columns.OrderId.IsForeignKey, Is.True);
        Assert.That(OrderItemMetadata.Columns.OrderId.ReferencedEntity, Is.EqualTo("Order"));
    }
    ...
    {
        var result = await _db.Insert(new User
        {
            UserName = "testuser",
            Email = "test@example.com",
            IsActive = true,
    ...
    public async Task InsertMany_MultipleEntities_ExecuteNonQueryAsync_Succeeds()
    {
        ...
        var result = await _db.InsertMany(entities).ExecuteNonQueryAsync();
    ...
    public async Task Insert_SingleEntity_ExecuteScalarAsync_ReturnsIdentity()
    {
        ...
        var identity = await _db.Insert(new User
        {
            UserName = "newuser",
    ...
    {
        var result = await _db.Insert(new Order
        {
            UserId = 1,
            Total = 99.99m,
            Status = "Pending",
    ...
    {
        var result = await _db.Delete<User>()
            .Where(u => u.UserId == 1)
            .ExecuteNonQueryAsync();
    ...
    public async Task VariableBasedDelete_ExecutesNonQuery()
    {
        ...
        var del = _db.Delete<User>().Where(u => u.UserId == 1);
    ...
    public async Task VariableBasedUpdate_ExecutesNonQuery()
    {
        ...
        var upd = _db.Update<User>().Set(u => u.UserName, "newname").Where(u => u.UserId == 1);
    ...
    public async Task ConditionalDelete_WithConditionTrue_IncludesConditionalWhere()
    {
        ...
        var del = _db.Delete<User>().Where(u => u.UserId > 100);
    ...
    public async Task ConditionalUpdate_WithConditionTrue_IncludesConditionalWhere()
    {
        ...
        var upd = _db.Update<User>().Set(u => u.UserName, "updated").Where(u => u.UserId == 1);
        if (true)
        {
