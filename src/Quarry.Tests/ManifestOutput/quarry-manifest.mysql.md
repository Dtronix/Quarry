...
```sql
INSERT INTO `events` (`EventName`, `ScheduledAt`, `CancelledAt`) VALUES (?, ?, ?)
```
...
```sql
SELECT `EventId`, `EventName`, `ScheduledAt`, `CancelledAt` FROM `events` WHERE `EventName` = ?
```
...
### Orders().Where(...).OrderBy(...).Select(...).Limit(...).ExecuteFetchAllAsync()
...
```sql
SELECT `d`.`UserName` FROM (SELECT DISTINCT `t0`.`UserName` AS `UserName`, (`t1`.`Total` + ?) AS `_o0` FROM `users` AS `t0` INNER JOIN `orders` AS `t1` ON `t0`.`UserId` = `t1`.`UserId` WHERE `t1`.`Total` > ?) AS `d` ORDER BY `d`.`_o0` ASC
```
...
```sql
SELECT `UserId`, `UserName`, `Email`, `IsActive`, `CreatedAt`, `LastLogin` FROM `users` ORDER BY `UserId` ASC LIMIT 18446744073709551615 OFFSET 1
```
...
```sql
-- base
SELECT `UserId`, `UserName`, `Email`, `IsActive`, `CreatedAt`, `LastLogin` FROM `users` WHERE `UserId` >= ? ORDER BY `UserId` ASC

-- +limitOn
SELECT `UserId`, `UserName`, `Email`, `IsActive`, `CreatedAt`, `LastLogin` FROM `users` WHERE `UserId` >= ? ORDER BY `UserId` ASC LIMIT ?
```
...
```sql
SELECT `UserName`, `Email`, `IsActive` FROM `users` WHERE `UserId` = ?
```
...
```sql
SELECT `UserId`, `UserName` FROM `users` WHERE `UserId` >= ? UNION SELECT `UserId`, `UserName` FROM `users` WHERE `UserId` <= ? ORDER BY `UserName` DESC LIMIT 2
```
...
```sql
SELECT * FROM (SELECT `UserId`, `UserName` FROM `users` WHERE `UserId` >= ? UNION SELECT `UserId`, `UserName` FROM `users` WHERE `UserId` <= ?) AS `__set` WHERE `UserId` = ?
```
...
```sql
WITH `Order` AS (SELECT `OrderId`, `UserId`, `Total`, `Status`, `Priority`, `OrderDate`, `Notes` FROM `orders` WHERE `Total` > 100), `User` AS (SELECT `UserId`, `UserName`, `Email`, `IsActive`, `CreatedAt`, `LastLogin` FROM `users` WHERE `IsActive` = ?) SELECT `OrderId`, `Total` FROM `Order`
```
...
```sql
SELECT `UserId` FROM `users` WHERE `UserName` LIKE CONCAT('%', ?, '%')
```
