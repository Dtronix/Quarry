...
public sealed class QuarryGenerator : IIncrementalGenerator
{
    ...
    private static void EmitFileInterceptorsNewPipeline(SourceProductionContext spc, FileInterceptorGroup group, Compilation compilation)
    {
        ...
        for (int i = 0; i < group.AssembledPlans.Count; i++)
        {
            ...
            if (assembled.Dialect == SqlDialect.MySQL)
            {
                // MySQL: a single pass rewrites bind-order markers to '?' (or collection
                ...
                // into manifests or generated SQL — with collection tokenization gated on
                // carrier eligibility exactly like the non-MySQL path below.
                RewriteMySqlBindMarkers(assembled, isCarrierEligible);
            }
    ...
    /// ranking, stored on <see cref="IR.AssembledPlan.MySqlBindOrder"/> when it differs
    /// from GlobalIndex (identity) order. Any validation failure falls back to identity —
    /// the pre-#303 bind behavior — rather than reordering on unreliable data; the
    /// rewritten (marker-free) SQL is always applied.
    /// </summary>
    private static void RewriteMySqlBindMarkers(IR.AssembledPlan assembled, bool isCarrierEligible)
    {
        ...
        var totalSlots = paramCount + (limitSlot != null ? 1 : 0) + (offsetSlot != null ? 1 : 0);
        ...
        foreach (var mask in maskKeys)
        {
            ...
            textOrder.Clear();
            var rewritten = IR.MySqlBindMarkers.RewriteAndExtract(variant.Sql, isCollectionSlot, textOrder);
            if (!ReferenceEquals(rewritten, variant.Sql))
                (updates ??= new List<(int, string, int)>()).Add((mask, rewritten, variant.ParameterCount));

            if (!valid || textOrder.Count == 0)
                continue;
            ...
            {
                if (slot < 0 || slot >= totalSlots || !seen.Add(slot)) { valid = false; break; }
            }
            if (!valid) continue;

            for (int i = 0; i < paramCount && valid; i++)
            {
            ...
            if (valid && limitSlot != null && !seen.Contains(limitSlot.Value)) valid = false;
            if (valid && offsetSlot != null && !seen.Contains(offsetSlot.Value)) valid = false;
            if (!valid) continue;
        ...
        if (updates != null)
        {
            foreach (var (key, sql, pc) in updates)
                assembled.SqlVariants[key] = new IR.AssembledSqlVariant(sql, pc);
        }

        if (!valid || master == null)
            return;
        ...
        if (offsetSlot != null && (tail == 0 || master[--tail] != offsetSlot.Value)) return;
        if (limitSlot != null && (tail == 0 || master[--tail] != limitSlot.Value)) return;
        if (tail != paramCount) return; // coverage gap — some param never rendered in any variant
    ...
    /// a bug (Debug.Assert) and aborts the merge. Slots never seen together (mutually
    /// exclusive conditional branches) are placed right after the previously matched slot;
    /// their relative bind order is immaterial since they never co-bind.
    /// </summary>
    private static bool TryMergeTextOrder(ref List<int>? master, List<int> incoming)
    {
        if (master == null)
        {
            ...
            if (pos >= 0)
            {
                if (pos <= lastPos)
                {
                    System.Diagnostics.Debug.Assert(false,
                        $"MySQL bind-order contradiction: slot {slot} has a different relative position across SQL variants.");
                    return false;
                }
                lastPos = pos;
            }
