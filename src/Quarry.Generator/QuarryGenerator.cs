...
public sealed class QuarryGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        // TEMP: emit With<> discovery trace
        context.RegisterImplementationSourceOutput(
            rawCallSites.Collect(),
            static (spc, sites) =>
            {
                var traces = GetWithTraces().ToArray();
                var sb = new System.Text.StringBuilder();
                sb.AppendLine("// <auto-generated/>");
                sb.AppendLine($"// TEMP: With<> discovery traces ({traces.Length} traces, {sites.Length} total raw sites)");
                foreach (var t in traces)
                    sb.AppendLine($"// {t}");
                sb.AppendLine($"// --- CTE sites found (checking {sites.Length} total sites) ---");
                foreach (var s in sites)
                {
                    if (s.MethodName is "With" or "FromCte")
                        sb.AppendLine($"// {s.Kind}: {s.MethodName} ctx={s.ContextClassName} entity={s.EntityTypeName} at {s.FilePath?.Split('\\').LastOrDefault()}:{s.Line}");
                }
                spc.AddSource("_Debug_WithTraces.g.cs", sb.ToString());
            });
    ...
    /// </summary>
    // TEMP: track With<> discovery results for debugging issue #205
    private static readonly System.Collections.Concurrent.ConcurrentBag<string> _withTraces = new();

    internal static System.Collections.Generic.IEnumerable<string> GetWithTraces() => _withTraces;
    ...
    private static ImmutableArray<IR.RawCallSite> DiscoverRawCallSites(GeneratorSyntaxContext context, CancellationToken ct)
    {
        ...
        try
        {
            // TEMP trace: capture With<> discovery path
            string? methodName = null;
            if (invocation.Expression is Microsoft.CodeAnalysis.CSharp.Syntax.MemberAccessExpressionSyntax ma)
            ...
            if (methodName == "With" && invocation.ArgumentList.Arguments.Count > 0)
            {
                ...
                var candidateDetails = string.Join("; ", symbolInfo.CandidateSymbols
                    .OfType<IMethodSymbol>()
                    .Select(ms => $"TP={ms.TypeParameters.Length} OTP={(ms.OriginalDefinition ?? ms).TypeParameters.Length} TA={ms.TypeArguments.Length} P={ms.Parameters.Length} {ms.Name}"));
                var typeArgCount = 0;
                if (invocation.Expression is Microsoft.CodeAnalysis.CSharp.Syntax.MemberAccessExpressionSyntax ma2
                    && ma2.Name is Microsoft.CodeAnalysis.CSharp.Syntax.GenericNameSyntax gn2)
                    typeArgCount = gn2.TypeArgumentList.Arguments.Count;
                var trace = $"With call: sym={symbolInfo.Symbol?.ToDisplayString() ?? "null"}, " +
                    $"candidates={symbolInfo.CandidateSymbols.Length}, " +
                    $"reason={symbolInfo.CandidateReason}, " +
                    $"explicitTA={typeArgCount}, " +
                    $"details=[{candidateDetails}], " +
                    $"file={invocation.SyntaxTree.FilePath?.Split('\\').LastOrDefault() ?? "?"}:{invocation.GetLocation().GetLineSpan().StartLinePosition.Line + 1}";
                _withTraces.Add(trace);
            }
