...
internal static class CarrierEmitter
{
    ...
    /// <summary>
    /// Emits a fully inlined scalar execution terminal body.
    /// Instead of delegating to QueryExecutor.ExecuteCarrierScalarWithCommandAsync,
    /// the generated method directly calls EnsureConnectionOpenAsync, ExecuteScalarAsync,
    /// and ScalarConverter.Convert inline — eliminating a method boundary, generic
    /// instantiation, and interface dispatch overhead.
    /// </summary>
    internal static void EmitInlineScalarTerminal(
        StringBuilder sb, CarrierPlan carrier, AssembledPlan chain)
    {
        // Carrier cast + context extraction
        sb.AppendLine($"        var __c = Unsafe.As<{carrier.ClassName}>(builder);");
        sb.AppendLine("        var __ctx = __c.Ctx!;");
        sb.AppendLine();

        // Connection open (benefits from the sync fast-path in Change A)
        sb.AppendLine("        await __ctx.EnsureConnectionOpenAsync(cancellationToken).ConfigureAwait(false);");
        sb.AppendLine();
        ...
        // Command creation + parameter binding
        var timeoutExpr = HasCarrierField(carrier, FieldRole.Timeout)
        ...
        // Reuse the existing parameter binding logic
        var dialectLiteral = GetDialectLiteral(chain.Dialect);
        var paramCount = chain.ChainParameters.Count;
        var hasLimitField = HasCarrierField(carrier, FieldRole.Limit);
        var hasOffsetField = HasCarrierField(carrier, FieldRole.Offset);
        ...
        // Instrumentation gate + Stopwatch start
        sb.AppendLine("        var __instrumented = LogsmithOutput.Logger != null || __ctx.SlowQueryThreshold.HasValue;");
        sb.AppendLine("        var __ts = __instrumented ? Stopwatch.GetTimestamp() : 0;");
        sb.AppendLine();

        // Try/catch wrapping the execution + conversion
        sb.AppendLine("        try");
        sb.AppendLine("        {");
        sb.AppendLine("            var __result = await __cmd.ExecuteScalarAsync(cancellationToken).ConfigureAwait(false);");
        sb.AppendLine();
        sb.AppendLine("            if (__instrumented)");
        sb.AppendLine("            {");
        sb.AppendLine("                var __elapsedMs = Stopwatch.GetElapsedTime(__ts).TotalMilliseconds;");
        sb.AppendLine("                var __threshold = __ctx.SlowQueryThreshold;");
        sb.AppendLine("                if (__threshold.HasValue && __elapsedMs > __threshold.Value.TotalMilliseconds)");
        sb.AppendLine("                {");
        sb.AppendLine("                    if (LogsmithOutput.Logger?.IsEnabled(LogLevel.Warning, ExecutionLog.CategoryName) == true)");
        sb.AppendLine("                        ExecutionLog.SlowQuery(__opId, __elapsedMs, __cmd.CommandText);");
        sb.AppendLine("                }");
        sb.AppendLine("            }");
        sb.AppendLine();
        ...
        sb.AppendLine("        }");
        sb.AppendLine("        catch (InvalidOperationException) { throw; }");
        sb.AppendLine("        catch (Exception ex) when (ex is not OperationCanceledException)");
        sb.AppendLine("        {");
        sb.AppendLine("            if (LogsmithOutput.Logger?.IsEnabled(LogLevel.Error, QueryLog.CategoryName) == true)");
        sb.AppendLine("                QueryLog.QueryFailed(__opId, ex);");
        sb.AppendLine();
        sb.AppendLine("            throw new QuarryQueryException($\"Error executing scalar query: {ex.Message}\", __cmd.CommandText, ex);");
        sb.AppendLine("        }");
    }
